...
import com.example.ledger.databinding.ActivityBillListBinding
import com.example.ledger.ui.adapter.BillAdapter
import com.example.ledger.viewmodel.BillViewModel
import java.io.File
...
class BillListActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel.loadBillsByMonth(currentMonth)
        viewModel.billsByMonth.observe(this) { bills ->
            adapter.submitList(bills ?: emptyList())
            binding.emptyHint.visibility = if (bills.isNullOrEmpty()) View.VISIBLE else View.GONE
        }

        binding.btnPrevMonth.setOnClickListener {
            val current = viewModel.currentMonth ?: return@setOnClickListener
            val prev = LocalDate.parse("$current-01").minusMonths(1).format(monthFormatter)
            updateMonthDisplay(prev)
            viewModel.loadBillsByMonth(prev)
        }

        binding.btnNextMonth.setOnClickListener {
            val current = viewModel.currentMonth ?: return@setOnClickListener
            val next = LocalDate.parse("$current-01").plusMonths(1).format(monthFormatter)
            updateMonthDisplay(next)
            viewModel.loadBillsByMonth(next)
        }
    ...
    private fun updateMonthDisplay(month: String) {
        try {
            val date = LocalDate.parse("$month-01")
            binding.tvMonth.text = "${date.year}年${date.monthValue}月"
        } catch (_: Exception) {
    ...
    private fun showEditDialog(bill: Bill) {
        ...
        // 分类
        viewModel.loadCategories(bill.type)
        ...
        val catRow = android.widget.LinearLayout(this).apply {
            ...
            addView(android.widget.TextView(this@BillListActivity).apply {
                text = "分类："; textSize = 14f
                setTextColor(ThemeManager.getColors(this@BillListActivity).textSecondary)
            })
            addView(android.widget.FrameLayout(this@BillListActivity).apply {
                layoutParams = android.widget.LinearLayout.LayoutParams(catIconSize, catIconSize)
        ...
        catRow.setOnClickListener {
            viewModel.categories.value?.let { cats ->
    ...
    private fun showAddCategoryAndPickDialog(
        ...
        val tvIcon = android.widget.TextView(this).apply {
            ...
            background = android.graphics.drawable.GradientDrawable().apply {
                setColor((ThemeManager.getColors(this@BillListActivity).primary and 0x00FFFFFF) or 0x15000000)
    ...
    private fun showIconPickerDialog(
        ...
        val selectedBg = android.graphics.drawable.GradientDrawable().apply {
            setColor((ThemeManager.getColors(this@BillListActivity).primary and 0x00FFFFFF) or 0x30000000)
            cornerRadius = 8 * density
        }
    ...
    private fun showTimeEditDialog(
        ...
        fun makeRow(labelText: String, value: String): EditText {
            val row = android.widget.LinearLayout(this@BillListActivity).apply {
                orientation = android.widget.LinearLayout.HORIZONTAL
                gravity = android.view.Gravity.CENTER_VERTICAL
                setPadding(0, dp(6), 0, 0)
            }
            row.addView(android.widget.TextView(this@BillListActivity).apply {
            ...
            val et = EditText(this@BillListActivity).apply {
        ...
        root.addView(android.widget.TextView(this).apply {
            ...
            setTextColor(ThemeManager.getColors(this@BillListActivity).primary)
        ...
        root.addView(android.view.View(this).apply {
            ...
            setBackgroundColor((ThemeManager.getColors(this@BillListActivity).primary and 0x00FFFFFF) or 0x20000000)
        })
